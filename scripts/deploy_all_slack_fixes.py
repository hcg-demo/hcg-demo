...
print("\n4. ServiceNow Lambda...")
deploy('hcg-demo-servicenow-action', 'src/lambda_servicenow_simple.py', 'lambda_function.py')
...
try:
    import subprocess
    subprocess.run(['python', 'scripts/infra/add_servicenow_ssm_permission.py'], check=False, capture_output=True, cwd='.')
    print("  OK SSM permissions")
except Exception as e:
    print(f"  Skip (run: python scripts/infra/add_servicenow_ssm_permission.py): {e}")
...
print("or SSM (/hcg-demo/servicenow/instance-url, username, password). Run scripts/infra/setup_servicenow.py")
print("=" * 60)
